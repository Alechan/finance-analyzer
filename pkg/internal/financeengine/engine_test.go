...
func TestEngine_Compute_WhenUsingDemoDataset_ThenCategoryBreakdownByMonthHasExpectedRows(t *testing.T) {
	...
	// Then
	require.Equal(t, 15, len(tbl.Rows), "one row per month+category aggregate")
	require.Equal(t, []string{"2025-03-01", "SUPERMARKET DEMO", "7777.00", "0.00", "38.35", "0.00"}, tbl.Rows[0])
	require.Contains(t, tbl.Rows, []string{"2025-02-01", "SUPERMARKET DEMO", "12444.00", "0.00", "63.14", "0.00"})
}
