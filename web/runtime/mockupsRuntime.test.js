...
test("loadRequiredFiles supports sensitive profile and loads every configured transaction CSV", async () => {
  ...
  const fetchImpl = async (path) => {
    ...
    return {
      ...
      text: async () => {
        ...
        if (path.endsWith(SENSITIVE_EXTRA_DATA_FILE)) return VALID_DATA_CSV;
        if (path.endsWith("VISA-PRISMA_joined.csv")) return VALID_DATA_CSV;
  ...
  assert.strictEqual(files.dataCsvFiles.length, 3);
  assert.ok(files.dataCsvFiles[0].name.includes("Santander"));
  assert.strictEqual(files.dataCsvFiles[1].name, SENSITIVE_EXTRA_DATA_FILE);
  assert.ok(files.dataCsvFiles[2].name.includes("VISA-PRISMA"));
  assert.strictEqual(files.categorySegmentsCsvText, VALID_CATEGORY_SEGMENTS_MAP);
  assert.deepStrictEqual(requested, [
    ...
    `/mockups/tmp_sensitive_data/current/${SENSITIVE_EXTRA_DATA_FILE}`,
    "/mockups/tmp_sensitive_data/current/VISA-PRISMA_joined.csv",
  ]);
});
